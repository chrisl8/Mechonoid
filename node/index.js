...
async function main() {
    ...
    for (const [key, value] of Object.entries(hardwareFunctions)) {
      ...
      if (
        ...
        if (value.type === 'RoboClaw' && value.device) {
          ...
          value.connection = new RoboClaw({
            comPort: value.device,
            myName: key,
          });
          ...
          value.connection.send({
            ...
            data: [value.minVoltage * 10, 15 * 10],
          });
          value.connection.send({
          ...
          value.connection.send({
            ...
            data: [value.maxCurrent * 100, 0, 0, 0, 0],
          });
          value.connection.send({
  ...
  // Joystick
  if (joystickConfigurations && joystickConfigurations.length > 0) {
    joystickInputHandler(joystickConfigurations);
  }
